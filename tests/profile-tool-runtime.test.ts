...
function runScenario(input: ScenarioInput) {
  const dir = mkdtempSync(join(tmpdir(), "opencode-mnemosyne-profile-runtime-"));
  tempDirs.push(dir);
...
const WARMUP_KEY = Symbol.for("opencode-mnemosyne.plugin.warmedup");
mkdirSync(join(tmpDir, ".opencode"), { recursive: true });
writeFileSync(
  join(tmpDir, ".opencode", "opencode-mnemosyne.json"),
...
const { MnemosynePlugin } = await import(${JSON.stringify(indexUrl)});
const plugin = await MnemosynePlugin({
  directory: tmpDir,
  worktree: tmpDir,
