...
function createRepoWithWorktree(): { repoDir: string; worktreeDir: string } {
  const repoDir = mkdtempSync(join(tmpdir(), "opencode-mnemosyne-scope-"));
...
describe("project scope identity", () => {
  ...
  it("uses different project tags for unrelated non-git directories", () => {
    const left = mkdtempSync(join(tmpdir(), "opencode-mnemosyne-left-"));
    const right = mkdtempSync(join(tmpdir(), "opencode-mnemosyne-right-"));
