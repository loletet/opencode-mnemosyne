...
describe("Windows Path Handling", () => {
  ...
  describe("path.join for cache directory", () => {
    it("should join paths correctly on Windows", () => {
      const storagePath = "C:\\Users\\user\\.opencode-mnemosyne";
    ...
    it("should join paths correctly on Unix", () => {
      const storagePath = "/home/user/.opencode-mnemosyne";
      const cacheDir = join(storagePath, ".cache");
      expect(cacheDir).toContain(".cache");
      expect(cacheDir).not.toContain("//");
    });
  ...
  describe("dirname for database path", () => {
    it("should extract directory correctly from Windows path", () => {
      const dbPath = "C:\\Users\\user\\.opencode-mnemosyne\\shards\\project.db";
      const dir = win32.dirname(dbPath);
      expect(dir).toBe("C:\\Users\\user\\.opencode-mnemosyne\\shards");
    });

    it("should extract directory correctly from Unix path", () => {
      const dbPath = "/home/user/.opencode-mnemosyne/shards/project.db";
      const dir = posix.dirname(dbPath);
      expect(dir).toBe("/home/user/.opencode-mnemosyne/shards");
    });
