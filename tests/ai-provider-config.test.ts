...
describe("AI provider config", () => {
  ...
  const originalLogFileEnv = process.env.OPENCODE_MNEMOSYNE_LOG_FILE;
  const logDir = mkdtempSync(join(tmpdir(), "opencode-mnemosyne-logs-"));
  const logFile = join(logDir, "opencode-mnemosyne.log");
  ...
  afterAll(() => {
    if (originalLogFileEnv === undefined) {
      ...
      process.env.OPENCODE_MNEMOSYNE_LOG_FILE = originalLogFileEnv;
    }
