...
export async function performAutoCapture(ctx, sessionID, directory, options = {}) {
    ...
    try {
        ...
        logger.info("auto-capture", "session messages loaded", {
            ...
            roles: messages.map((m) => m.info?.role ?? "unknown").join(","),
        });
...
async function generateSummary(context, sessionID, userPrompt) {
    ...
    if (CONFIG.opencodeProvider && CONFIG.opencodeModel) {
        ...
        if (!isProviderConnected(CONFIG.opencodeProvider)) {
            logger.error("auto-capture.inference", "opencode provider is not connected", new Error(`opencode provider '${CONFIG.opencodeProvider}' is not connected`), {
                sessionID,
                providerID: CONFIG.opencodeProvider,
                modelID: CONFIG.opencodeModel,
            });
        ...
        if (!v2Client) {
            logger.error("auto-capture.inference", "opencode v2 client is not initialized", new Error("v2 client not initialized"), {
                sessionID,
                providerID: CONFIG.opencodeProvider,
                modelID: CONFIG.opencodeModel,
            });
            throw new Error("opencode-mnemosyne: v2 client not initialized; cannot perform structured-output capture");
        }
...
Analyze this conversation. If it contains technical work (code, bugs, features, decisions), create a concise summary and relevant tags. If it's non-technical (greetings, casual chat, incomplete requests), return type="skip" with empty summary.`;
    ...
    if (!CONFIG.memoryModel || !CONFIG.memoryApiUrl) {
        logger.error("auto-capture.inference", "external API configuration missing", new Error("External API not configured for auto-capture"), {
            sessionID,
            memoryProvider: CONFIG.memoryProvider,
            memoryModel: CONFIG.memoryModel,
            hasMemoryApiUrl: Boolean(CONFIG.memoryApiUrl),
