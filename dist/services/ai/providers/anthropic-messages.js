        ...
        while (iterations < maxIterations) {
            ...
            try {
                ...
                if (data.stop_reason === "end_turn") {
                    logger.info("auto-capture.inference.http", "anthropic response ended without tool use; retrying", {
                        sessionId,
                        aiSessionId: session.id,
                        iteration: iterations,
                    });
                ...
                clearTimeout(timeout);
                logger.error("auto-capture.inference.http", "anthropic executeToolCall iteration failed", error, {
                    sessionId,
                    iteration: iterations,
                    model: this.config.model,
                    apiUrl: this.config.apiUrl,
