...
export const MnemosynePlugin: Plugin = async (ctx: PluginInput) => {
  const { directory } = ctx;
    ...
    event: async (input: { event: { type: string; properties?: any } }) => {
      ...
      if (event.type === "session.idle") {
        ...
        idleTimeout = setTimeout(async () => {
          try {
            ...
            log("Idle processing error", { error });
          } finally {
            idleTimeout = null;
          }
        }, 10000);
      ...
      if (event.type === "session.compacted") {
        ...
        try {
          ...
          log("Compaction handler error", { error });
        }
      }
    },
  };
