...
const CONFIG_FILES = [
  ...
  join(CONFIG_DIR, "opencode-mnemosyne.json"),
];
...
function loadConfigFromPaths(paths: string[]): MnemosyneConfig {
...
const CONFIG_TEMPLATE = `{
  // ============================================
  // Mnemosyne Plugin Configuration
  // ============================================
...
function ensureConfigExists(): void {
  const configPath = join(CONFIG_DIR, "opencode-mnemosyne.jsonc");

  if (!existsSync(configPath)) {
    try {
      writeFileSync(configPath, CONFIG_TEMPLATE, "utf-8");
      console.log(`\n✓ Created config template: ${configPath}`);
      console.log("  Edit this file to customize mnemosyne settings.\n");
    } catch {}
  }
}
...
function buildConfig(fileConfig: MnemosyneConfig) {
  return {
...
export function initConfig(directory: string): void {
  const projectPaths = [
    ...
    join(directory, ".opencode", "opencode-mnemosyne.json"),
  ];
