...
function formatEntry(entry: LogEntry): string {
  ...
  const inline: string[] = [];
  const multiline: Array<[string, unknown]> = [];
...
function isSimple(value: unknown): value is string | number | boolean {
...
export function parseEntries(text: string): LogEntry[] {
  ...
  const flush = (): void => {
    ...
    if (!parsed) {
      ...
      const context: LogContext = { ...parsed.context };
...
function parseHeader(header: string): {
  ...
  if (old) {
    return {
      ...
      level: (old[2] ?? "INFO").toLowerCase() as LogLevel,
      scope: old[3] ?? "app",
      message: old[4] ?? "",
      context: {},
    };
  ...
  const level = (pretty[1] ?? "INFO").toLowerCase() as LogLevel;
...
function parsePrettyMessageAndInlineContext(text: string): {
  message: string;
  context: LogContext;
} {
...
function parseScalar(valueText: string): string | number | boolean {
  if (valueText === "true" || valueText === "false") return valueText === "true";
...
function parseErrorBlock(text: string): SerializedError {
  ...
  if (trimmed.startsWith("{")) {
    const parsed = JSON.parse(trimmed) as SerializedError;
    return parsed;
  }
